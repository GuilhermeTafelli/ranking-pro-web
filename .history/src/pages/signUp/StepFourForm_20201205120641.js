...
export default function StepFourForm() {
    ...
    function handleSubmitSkill() {
        setSkill("")
        if (skill) {
            setSkills({values: [...skills.values, skill]})
        }
    }
    ...
    function handleSubmitNiche() {
        setNiche("")
        if (niche) {
            setNiches({values: [...niches.values, niche]})
        }
    }
    ...
    function handleDeleteSkill(item) {
        var newSkills = skills.values
        newSkills.splice(skills.values.indexOf(item), 1);
        setSkills({values: newSkills})
    }

    function handleDeleteNiche(item) {
        var newNiches = niches.values
        newNiches.splice(niches.values.indexOf(item), 1);
        setNiches({values: newNiches})
    }
            ...
            <Grid container>
                    ...
                    <Grid item>
                        <Grid item className={classes.chipContainerItem} xs={12}>
                            <Grid container spacing={1}>
                                {skills.values && skills.values.map(item =>
                                    <Grid item>
                                        <Chip label={item} onDelete={() => handleDeleteSkill(item)} />
                                    </Grid>
                                )}
                            </Grid>
                        </Grid>
                ...
                <Grid container item className={classes.input} xs={12}>
                    ...
                    <Grid item>
                        <Grid item className={classes.chipContainerItem} xs={12}>
                            <Grid container spacing={1}>
                                {niches.values && niches.values.map(item =>
                                    <Grid item>
